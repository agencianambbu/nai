<script lang="ts">
	...
	onMount(async () => {
		if ($user.role === 'admin') {
			...
			const imageConfigRes = await getImageGenerationConfig(localStorage.token).catch(
				(error) => {
					toast.error(error);
					return null;
				}
...
>
								...
								on:change={(e) => {
									...
									if (enabled) {
										if (
											...
											toast.error(
												$i18n.t('AUTOMATIC1111 Base URL is required.')
											);
								...
								{#each requiredWorkflowNodes as node}
									...
									>
										<div class="flex-shrink-0">
											<div
												class=" capitalize line-clamp-1 font-medium px-3 py-1 w-20 text-center rounded-l-lg bg-orange-500/10 text-orange-700 dark:text-orange-200"
											>
										...
										<div class="">
											<Tooltip
												content="Input Key (e.g. text, unet_name, steps)"
											>
												<input
	...
	<div class="flex justify-end pt-3 text-sm font-medium">
		<button
			class=" px-4 py-2 bg-orange-700 hover:bg-orange-800 text-gray-100 transition rounded-lg flex flex-row space-x-1 items-center {loading
				? ' cursor-not-allowed'
				: ''}"
			type="submit"
			disabled={loading}
