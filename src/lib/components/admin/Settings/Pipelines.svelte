<script lang="ts">
	...
	const uploadPipelineHandler = async () => {
		...
		if (pipelineFiles && pipelineFiles.length !== 0) {
			...
			const res = await uploadPipeline(
				localStorage.token,
				file,
				selectedPipelinesUrlIdx
			).catch((error) => {
...
>
			...
			{#if PIPELINES_LIST.length > 0}
							...
							>
								...
								{#each PIPELINES_LIST as pipelines, idx}
									<option
										...
										class="bg-gray-100 dark:bg-gray-700">{pipelines.url}</option
									>
								{/each}
							</select>
						</div>
				...
				<div class=" my-2">
					...
					<div class="flex w-full">
						<div class="flex-1 mr-2">
							...
							>
								{#if pipelineFiles}
									{pipelineFiles.length > 0 ? `${pipelineFiles.length}` : ''} pipeline(s)
									selected.
								{:else}
				...
				<div class=" my-2">
					...
					<div class="mt-2 text-xs text-gray-500">
						<span class=" font-semibold dark:text-gray-200">Warning:</span> Pipelines
	...
	<div class="flex justify-end pt-3 text-sm font-medium">
		<button
			class=" px-4 py-2 bg-orange-700 hover:bg-orange-800 text-gray-100 transition rounded-lg"
			type="submit"
		>
			{$i18n.t('Save')}
		</button>
