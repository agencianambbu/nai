	...
	<div class="mx-auto w-full md:px-0 h-full">
		<div class=" flex flex-col h-full">
					...
					<div>
						<button
							class=" flex items-center gap-0.5 text-xs px-2.5 py-0.5 rounded-lg {mode ===
								'chat' && 'text-sky-600 dark:text-sky-200 bg-sky-200/30'} {mode ===
								'complete' &&
			...
			<div class="pb-3">
				{#if !loading}
					<button
						class="px-3 py-1.5 text-sm font-medium bg-orange-600 hover:bg-orange-700 text-gray-50 transition rounded-lg"
						on:click={() => {
							submitHandler();
						}}
					>
