<script>
	...
	const submitHandler = async () => {
		...
		const res = await updateMemoryById(localStorage.token, memory.id, content).catch(
			(error) => {
				toast.error(error);
			...
			<div class=" flex flex-col w-full sm:flex-row sm:justify-center sm:space-x-6">
				...
				>
					...
					<div class="flex justify-end pt-1 text-sm font-medium">
						<button
							class=" px-4 py-2 bg-orange-700 hover:bg-orange-800 text-gray-100 transition rounded-3xl flex flex-row space-x-1 items-center {loading
								? ' cursor-not-allowed'
								: ''}"
							type="submit"
							disabled={loading}
